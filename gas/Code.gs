...
const HISTORY_HEADERS = ['日時', '操作', '商品名', '仕入元', '保管場所', '変更前', '変更後', 'メモ'];
...
function addItem(item) {
  ...
  logHistory('追加', item.商品名, item.仕入元, item.保管場所, '', qty, '');
  return { id: id };
}
...
function updateItem(item) {
  ...
  logHistory('編集', item.商品名, item.仕入元, item.保管場所, old.在庫数量, Number(item.在庫数量) || 0, memo);
  return { id: item.id };
}
...
function deleteItem(id) {
  ...
  const old = rowToObj(sheet.getRange(r, 1, 1, HEADERS.length).getValues()[0]);
  sheet.deleteRow(r);
  logHistory('削除', old.商品名, old.仕入元, old.保管場所, old.在庫数量, '', '');
  return { id: id };
}
...
function adjustQty(id, delta) {
  ...
  logHistory(delta > 0 ? '入庫' : '出庫', item.商品名, item.仕入元, item.保管場所, cur, next, `${delta > 0 ? '+' : ''}${delta}${item.単位}`);
...
function moveItem(req) {
  ...
  logHistory('移動', src.商品名, src.仕入元, `${src.保管場所}→${req.to_location}`, src.在庫数量, src.在庫数量 - qty, `${qty}${src.単位}を${req.to_location}へ`);
  return { ok: true };
}
...
function logHistory(op, name, supplier, location, before, after, memo) {
  getHistorySheet().appendRow([new Date(), op, name || '', supplier || '', location || '', before !== undefined ? before : '', after !== undefined ? after : '', memo || '']);
}
